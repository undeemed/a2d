...
> Detect & gate (config-only, no GPU), the conversion recipe (GPT-2, the GQA+RoPE family: Gemma 1, Qwen2, Llama, and the sliding-window family: Gemma 2/3, Mistral), the identity gate, sampling, and the eval harness run end to end.
...
| Sliding-window models (Mistral, Gemma 2/3…) | Attention-sink / MXFP4-quantized models (GPT-OSS) — *planned, capability-gated* |
|  | ONNX-only exports (`format`) |
...
Phased, no dates: walking skeleton → detection & gate (no GPU) → dense conversion (GPT-2, then Gemma 1/Qwen2/Llama) → eval harness → MoE (OLMoE) → block diffusion & fast sampling → hard architectures (SWA landed: Mistral, Gemma 2/3; sinks/quantized GPT-OSS remain) → polish. Details and exit criteria: [`docs/SPEC-HANDOFF.md`](docs/SPEC-HANDOFF.md).
